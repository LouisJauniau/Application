...
class _CarteScreenState extends State<CarteScreen> {
  ...
  List<SavedElement> savedElements = [SavedElement('', Colors.white, '10')];

  void majDataUser() async {
    ...
    List<String> myList = [];
    await dataUser.forEach((key, value) {
      if (value is List) {
        if (key != 'favoris') myList.add(key);
      }
    });

    // Récupération du champ de type Map à partir du document
    var mapResultat = dataUser['couleurs'] as Map<String, dynamic>;

    List<SavedElement> listeParcours = [];

    if (mapResultat != null && mapResultat.isNotEmpty) {
      for (int i = 0; i < myList.length; i++) {
        await mapResultat.forEach((key, value) async {
          if (key == myList[i]) {
            listeParcours.add(SavedElement(myList[i], stringToColor(value),
      ...
      for (int i = 0; i < myList.length; i++) {
        listeParcours.add(SavedElement(
            myList[i], Colors.black, await parcoursFunction(myList[i])));
      }
    }

    if (!mounted) return;
    setState(() {
      savedElements = listeParcours;
    });
  }
  ...
  Future<List<String>> getArrayFieldNames() async {
    ...
    try {
      ...
      await documentData.forEach((key, value) {
        if (value is List) {
  ...
  Future<Map<String, dynamic>> getMapFields() async {
    ...
    // Récupération du champ de type Map à partir du document
    final mapData = documentSnapshot.data()['couleurs'] as Map<String, dynamic>;
  ...
  Widget build(BuildContext context) {
    ...
    //recupere la valeur du champs couleurs
    FutureBuilder(
      ...
      builder:
        ...
        if (snapshot.connectionState == ConnectionState.done) {
          ...
          if (data.containsKey("couleurs")) {
            Map<String, dynamic> couleurs =
            data["couleurs"] as Map<String, dynamic>;
            for (String fieldName in couleurs.keys) {
              if (data.containsKey(fieldName)) {
                List<dynamic> markers = data[fieldName];
                for (var i = 0; i < markers.length; i++) {
                  markers[i].couleur = couleurs[fieldName];
                  print('couleur');
                  list[i].couleur = couleurs[fieldName];
                }
              }
            }
          }
    ...
    // Map<String, dynamic> userdata =   getAllData();
    // print("userdata");
    // fais moi une variable userData qui appelle getAllData() et qui print ensuite la donnee
    Future<void> fetchData() async {
      var userData = await getAllData();
      print(userData);
    }
    ...
    void openMarker(CustomMarker item) {
      setState(() {
        if (widget.auth) {
          _position = _height - 200 - kBottomNavigationBarHeight;
          if (item.hasPhoto) _position -= 200; // 2024 : 01-photo2
    ...
    buildAllMarker = balises.buildAllMarker(dataUser);
              ...
              children: [
                ...
                MarkerClusterLayerWidget(
                  options: MarkerClusterLayerOptions(
                    ...
                    markers: buildAllMarker, // Your existing markers
                    builder: (context, markers) {
                      return Stack(
                        clipBehavior: Clip.none,
                        children: [
                          Image.asset('assets/images/marker/marqueurVioletFoncé.png', width: 50),
                          Positioned(
                            top: 8,
                            left: 0,
                            right: 0,
                    ...
                    showPolygon: true, // Shows connector lines
                  ),
                ),
              ],
            ),
